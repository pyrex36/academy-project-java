...
public class MonopolyController implements GameController {
    ...
    public void startGame() {
        ...
        mIOHandler.writeMessage("Players:");
        mIOHandler.writeMessage("===================");

        for (MonopolyPlayer player : mPlayers) {
            player.setMoney(DEFAULT_MONEY);
            mIOHandler.writeMessage(player.toString());
        }
    }
    ...
    public void step() {
        ...
        for (MonopolyPlayer player : activePlayers) {
            int playerRoll = mDie.rollMultiple(NUMBER_OF_DICE);
            ...
            MonopolyTile tile = mTiles.get(player.getPosition());
    ...
    @Override
    public void endGame() {
        MonopolyPlayer winner = mPlayers.stream()
    ...
    public boolean isPropertyAssigned(MonopolyProperty property) {
    ...
    public List<MonopolyProperty> getPropertiesForPlayer(MonopolyPlayer player) {
        return mPropertyToPlayerMap.entrySet()
                .stream()
