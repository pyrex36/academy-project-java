    ...
    private static void printInfo(MonopolyController controller) {
        for (MonopolyPlayer p: controller.getPlayers()) {
            ...
            for (MonopolyProperty property  : properties) {
                StdInOutHandler.INSTANCE.writeMessage("  %s has %d houses, cost of house %d, buyback %d".formatted(property.name(), property.numOfHouses(), property.costOfHouse(), property.buybackRate()));
            }
        }
    }
}
